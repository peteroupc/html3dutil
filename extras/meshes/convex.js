 ...
 * @returns {*} A buffer geometry.  The generated convex hull.
 * @function
 */
export const createConvexHull = function(three, points, flat, inside) {
  const bm = new QuickHull();
  bm.buildMesh(points, 1e-8);
  const mvi = bm.mesh.toMeshVerticesIndices(points);
  var ret= Meshes.fromPositions(mvi[0],mvi[1]);
  return Meshes.recalcNormals(ret);
};
 ...
 * @returns {*} A buffer geometry.  The resulting polyhedron.
 * @function
 */
