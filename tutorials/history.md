...
- All classes in the main library are moved to a new namespace called `H3DU`.  For example, `Shape` is now {@link Shape} and `Mesh` is now {@link Mesh}.  Many classes in the "extras" directory are also moved to the `H3DU` namespace.
- `Scene3D`, now {@link Scene3D}, is no longer meant to be a scene graph of objects to draw. That job now belongs to the new {@link Batch3D} class. Scene3D's `render` method now takes an array of `Batch3D`s to render. For compatibility, though, the methods allowing it to manage 3D models and the coordinate system, such as `makeShape` and `setPerspective`, can still be used until `H3DU.Scene3D` renders a custom `H3DU.Batch3D`. This compatibility behavior may be dropped in the future.
- Alpha is disabled in WebGL contexts created with the {@link get3DOr2DContext} method.
- The `Scene3D` {@link Scene3D#useProgram} method was deprecated and now does nothing.
- New {@link RenderPass3D} class holds information about how a batch of 3D models is to be rendered. It replaces the `Scene3D` {@link Scene3D#useFilter} method, which now does nothing.
- New {@link FrameBufferInfo} class holds information about a frame buffer; it replaces {@link FrameBuffer}.
- The `BufferedMesh`, `FrameBuffer`, and `ShaderProgram` classes are deprecated because they are too tightly coupled with a particular WebGL context. Instead, use {@link MeshBuffer}, {@link FrameBufferInfo}, and {@link ShaderInfo}, respectively, which are not coupled to WebGL contexts.
- Rendering can make use of vertex array objects internally, if supported by the WebGL implementation.
- The {@link Shape} object is no longer coupled to vertex buffers.
- The {@link LightSource} class now supports a radius of the light.
- The {@link TextureLoader} class was added for loading textures; a single object of this class can load and upload images from multiple WebGL contexts. This is unlike `BufferedMesh`, `FrameBuffer`, and `ShaderProgram`, which are tied to the WebGL context.
- `GLMath`, now {@link Math}, was expanded with many new methods. The documentation for it is now very detailed. New methods include {@link MathUtil.vec3perp}, {@link MathUtil.vec3toWindowPoint}, and {@link MathUtil.mat4projectVec3}.
- Two new classes in the "extras" folder support 2D text rendering and texture atlases (as sprite sheets), namely, {@link TextFont} and {@link TextureAtlas}.
- The "doc" folder contains the documentation to the library in the form of Markdown text files.
- The Camera class, now {@link Camera}, was rewritten.
...
- Added `getBounds` and `toLinePath` methods to {@link GraphicsPath}, an extra, as well
...
- The default light configuration is no lights when creating a {@link LightSource}. The exception, for compatibility purposes, is when using a {@link Scene3D} without rendering a custom `Batch3D`, in which case the default is one light source with its default values.
