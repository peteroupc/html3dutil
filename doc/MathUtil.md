...
a rotation from one vector to another; <a href="MathUtil.md#MathUtil.quatFromMat4">MathUtil.quatFromMat4</a>, which generates a quaternion from a [\*\*4x4 matrix\*\*](#Matrices); <a href="MathUtil.md#MathUtil.quatFromAxisAngle">MathUtil.quatFromAxisAngle</a>, which generates a quaternion from an angle and [\*\*axis of rotation\*\*](#Axis_of_Rotation); <a href="MathUtil.md#MathUtil.quatFromTaitBryan">MathUtil.quatFromTaitBryan</a>, which generates a quaternion from Tait-Bryan angles.
...
- Normalize the rotation quaternion (using <a href="MathUtil.md#MathUtil.quatNormalize">`quatNormalize()`</a>
...
Related functions:
<a href="MathUtil.md#MathUtil.quatFromTaitBryan">MathUtil.quatFromTaitBryan()</a> -
Converts from Tait-Bryan angles to a quaternion
<a href="MathUtil.md#MathUtil.quatToTaitBryan">MathUtil.quatToTaitBryan()</a> -
...
<a href="MathUtil.md#MathUtil.boxDimensions">MathUtil.boxDimensions</a> - Finds a box's dimensions.
...
* [mat4invert](#MathUtil.mat4invert)<br>Finds the inverse of a 4x4 matrix.
...
transformed by this matrix and then converted to glmath. But if the
...
See "Axis of Rotation" in "glmath" for the meaning of each angle.
...
in the second vector. The vectors need not be glmath.
...
If both quaternions are glmath, the resulting
...
* `a` (Type: Array.&lt;number>)<br>A quaternion. Should be a glmath.
...
See "Axis of Rotation" in "glmath" for the meaning of each angle. (Type: Array.&lt;number>)
...
<li>If A and B are glmath, the length equals the absolute value
...
<li>If both vectors are glmath, the cosine
